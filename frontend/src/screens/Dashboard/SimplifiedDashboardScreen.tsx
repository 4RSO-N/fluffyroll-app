...
export default function SimplifiedDashboardScreen() {
  const { user } = useAuth();
  ...
  const handleLogWater = async () => {
    try {
      ...
      Alert.alert('Success', 'Water logged successfully! 💧'); // Mock success
      setWaterModalVisible(false);
      setWaterGlasses('1');
      loadDashboard();
    }
  ...
  const handleLogMeal = async () => {
    ...
    try {
      ...
      Alert.alert('Success', 'Meal logged successfully! 🍽️'); // Mock success
      setMealModalVisible(false);
      setMealName('');
      setCalories('');
      setProtein('');
  ...
  const calculateHealthScore = () => {
    ...
    const overallScore = (workoutScore * 0.4) + (caloriesScore * 0.4) + (waterScore * 0.2);
      ...
      <View style={styles.section}>
        <View style={styles.healthScoreCard}>
          ...
          <View style={styles.scoreContainer}>
            <Text style={[styles.scoreValue, { color: healthScore >= 70 ? Colors.success : Colors.warning }]}>
              {healthScore}
            </Text>
            <Text style={styles.scoreLabel}>/ 100</Text>
          </View>
          <Text style={styles.scoreDescription}>
            {healthScore >= 85 ? 'Excellent!' : healthScore >= 70 ? 'Good' : healthScore >= 50 ? 'Fair' : 'Needs Improvement'}
          </Text>
        </View>
      </View>
